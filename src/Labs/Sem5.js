          ...
          return (
            <div className="overflow-hidden  hover:skew-x-12 transition-all duration-500 hover:font-bold">
              <a
                href={item.link}
                target="_blank"
                className=" flex-col text-2xl bg-yellow-700 m-3 rounded-md w-[120px] h-[120px] flex justify-center items-center mx-auto  "
              >
                <h1>{subjectName}</h1>
              </a>
              <p className=" text-center">
                shared by{" "}
                <span className=" text-red-500">{StduentDetails}</span>
              </p>
            </div>
          );
        })}
