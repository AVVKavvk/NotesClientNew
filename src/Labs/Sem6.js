...
function Sem6() {
  ...
  const getData = async () => {
    try {
      const result = await axiosClient.post("/sem6/get/lab", { sem: 1 });
    ...
    <div class="min-h-screen  mb-5  mt-7 overflow-hidden ">
      ...
      <div class="mx-auto  md:max-w-[1000px] flex pl-10  justify-evenly items-center flex-col  mt-8 ">
        {data?.map(item => {
          ...
          return (
            <div class="flex  gap-5 px-2 justify-center items-center  mt-10  ">
              <div>
                <a href={item.link} target="_blank">
                  <h1 class=" bg-orange-400  hover:scale-110    text-white rounded-md  w-[120px] h-[120px] text-center flex justify-center items-center transition-all duration-500 text-4xl mt-3 ">
                    {item.subject}
                  </h1>
                </a>
              </div>
