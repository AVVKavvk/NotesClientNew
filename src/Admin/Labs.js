...
function Labs() {
  ...
  async function GetLabs(e) {
    e.preventDefault();
    try {
      const result = await axiosClient.post(`/sem${sem}admin/labs`, {
        adminEmail: email,
        adminPassword: password,
        token,
      });
      setLabs(result.result || []);
  ...
  async function UpdateLab(e) {
    ...
    //   console.log(updateId);
      ...
      const result = await axiosClient.put(`/sem${sem}admin/labs`, {
        adminEmail: email,
        adminPassword: password,
        token,
        id: updateId,
  ...
  async function DeleteLab(e) {
    e.preventDefault();
    try {
      const result = await axiosClient.delete(`/sem${sem}admin/labs`, {
        data: {
          adminEmail: email,
          adminPassword: password,
          token,
      ...
      <div className="flex gap-8 flex-wrap h-min-[100vh] pb-16 mx-auto px-6 justify-center items-center">
        {labs?.map(lab => {
          return (
            ...
            >
              <h1 className=" text-xl">{lab?.subject_name}</h1>
              <h1>{lab?.studentEmail}</h1>
              <h1>{lab?._id}</h1>
              <a
                href={lab?.link}
                target="_blank"
